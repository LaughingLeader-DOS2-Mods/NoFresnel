...
vertex VertexOutput Characters_PBR_Characters_PBR_Anisotropic_AlphaTest_2S_SK_DEF_Metal_vertexMain(constant LocalUniformsVS& uniforms [[buffer(5)]],
	...
	VertexInput In [[stage_in]])
{
	...
	float3 viewSpaceTangent = (float3x3(perView.global_View[0].xyz, perView.global_View[1].xyz, perView.global_View[2].xyz) * worldTangentNormalized);

	Out.ViewTangent = viewSpaceTangent;
...
fragment PixelOutput Characters_PBR_Characters_PBR_Anisotropic_AlphaTest_2S_SK_DEF_Metal_fragmentMain(constant LocalUniformsPS& uniforms,
	...
	texture2d<float> Texture2DParameter_physicalmap_DefaultWrapSampler)
{
	...
	gBufferData.ShadingModel = 2;
	float3 viewTangentNormalized = normalize(In.ViewTangent);

	gBufferData.Custom.xyz = EncodeTangents(PatchTangent(matNormal, viewTangentNormalized));
	gBufferData.Custom.w = matAnisotropy;
